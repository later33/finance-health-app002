/* ============================================================
   supabase.js · Supabase 客户端 + 认证 + 数据层
   替代原来的 finance-data.js localStorage 方案
   ============================================================ */
...
async function dbSaveSnapshot() {
  ...
  const label  = new Date().toISOString().slice(0, 7); // "2025-06"

  // 同月只保留最新：先删再插
  await sb.from('net_worth_history')
    .delete()
    .eq('user_id', user.id)
    .eq('date_label', label);
...
async function dbLoadHistory() {
  ...
  if (error) { console.error(error); return []; }
  return data.map(r => ({ date: r.date_label.slice(5).replace('-','月')+'月', netWorth: r.net_worth }));
}
